...
const changeQuantity = (id,value)=> {
  ...
  let updateCart= cart.map((item)=>{
   ...
   let newqantity = item.quantity + value;// this execute when we click button 
   newqantity = newqantity > 0 ? newqantity:0;//this line is for when we press "-" button after reaching 0 also it gives minus values linke -1 so on to aviod it how many times we press "-" button after reaching 0 it will be in 0 only
  return  item.id=== id ? {...item ,quantity:newqantity}:item //here quantity value will be changed in item object and create copy and again asigned to same object name "item"
    ...
    <div style={ {backgroundColor: "black",color:"gray"}}>
      {
        cart.map((item)=>{
            return(
                <CartItems key={item.id} {...item} changeQuantity={changeQuantity}/>  // this entire thing is <CartItems/> 
            )
          })
      }
